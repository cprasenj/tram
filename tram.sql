...
CREATE TABLE customer (
	...
	CONSTRAINT customer_pk PRIMARY KEY(employee_id)
);
...
CREATE TABLE deport (
	...
	CONSTRAINT deport_id_fk FOREIGN KEY (deport_id) REFERENCES deport(deport_id)
);
...
CREATE TABLE tram_type (
	tram_type_id 		NUMBER(3),
	tram_type_name		VARCHAR(20),
	fare_chart_id		NUMBER(5),
	CONSTRAINT tram_pk PRIMARY KEY(tram_type_id)
);
...
CREATE TABLE tram (
	...
	CONSTRAINT tram_type_fk FOREIGN KEY (tram_type_id) REFERENCES tram_type(tram_type_id)    
);
